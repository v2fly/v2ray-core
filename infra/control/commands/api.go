...
func (c *APICommand) Description() command.Description {
	return command.Description{
		...
		Usage: []string{
			...
			"Examples:",
			fmt.Sprintf("  %s %s --server=127.0.0.1:8080 LoggerService.RestartLogger '' ", command.ExecutableName, c.Name()),
			fmt.Sprintf("  %s %s --server=127.0.0.1:8080 StatsService.QueryStats 'pattern: \"\" reset: false'", command.ExecutableName, c.Name()),
			fmt.Sprintf("  %s %s --server=127.0.0.1:8080 StatsService.GetStats 'name: \"inbound>>>statin>>>traffic>>>downlink\" reset: false'", command.ExecutableName, c.Name()),
			fmt.Sprintf("  %s %s --server=127.0.0.1:8080 StatsService.GetSysStats ''", command.ExecutableName, c.Name()),
		},
	}
}
