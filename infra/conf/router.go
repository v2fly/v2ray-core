...
func (c *RouterConfig) getDomainStrategy() router.Config_DomainStrategy {
	...
	case "alwaysip", "always_ip":
		return router.Config_UseIp
	case "ipifnonmatch", "ip_if_non_match":
		return router.Config_IpIfNonMatch
	case "ipondemand", "ip_on_demand":
...
func loadIP(filename, country string) ([]*router.CIDR, error) {
	...
	newError("country not found in ", filename, ": ", country)
	return nil, nil
}
...
func loadSite(filename, country string) ([]*router.Domain, error) {
	...
	newError("list not found in ", filename, ": ", country).AtWarning().WriteToLog()
	return nil, nil
}
...
func loadGeositeWithAttr(file string, siteWithAttr string) ([]*router.Domain, error) {
	...
	if len(parts) == 0 {
		return nil, newError("empty rule: ", siteWithAttr)
	}
	...
	if attrs.IsEmpty() {
		if strings.Contains(siteWithAttr, "@") {
			newError("empty attribute list: ", siteWithAttr).AtWarning().WriteToLog()
		}
		return domains, nil
	}
	...
	if !hasAttrMatched {
		newError("all attributes match no rule: geosite:", siteWithAttr).AtWarning().WriteToLog()
	}
...
func parseDomainRule(domain string) ([]*router.Domain, error) {
	...
	if isExtDatFile != 0 {
		...
		if err != nil {
			return nil, newError("failed to load external list: ", country, " from ", filename).Base(err)
		}
	...
	case strings.HasPrefix(domain, "domain:"):
		domainName := domain[7:]
		if len(domainName) == 0 {
			return nil, newError("empty sub domain type of rule: ", domain)
		}
