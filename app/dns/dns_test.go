...
func (*staticHandler) ServeDNS(w dns.ResponseWriter, r *dns.Msg) {
	...
	for _, q := range r.Question {
		...
		case q.Name == "only.google.com." && q.Qtype == dns.TypeA:
			rr, _ := dns.NewRR("only.google.com. IN A 8.8.8.8")
			ans.Answer = append(ans.Answer, rr)

		case q.Name == "only.google.com." && q.Qtype == dns.TypeAAAA:
			rr, _ := dns.NewRR("only.google.com. IN AAAA 2001:4860:4860::8888")
			ans.Answer = append(ans.Answer, rr)
...
func TestStaticHostDomainWithIP4Only(t *testing.T) {
	port := udp.PickPort()
	...
	{
		ips, err := client.LookupIP("only.google.com")
		t.Log(ips)
		if err != nil {
			t.Fatal("unexpected error: ", err)
		}
...
func TestStaticHostDomainWithIP6Only(t *testing.T) {
	port := udp.PickPort()
